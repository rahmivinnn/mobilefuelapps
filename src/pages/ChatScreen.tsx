...
const ChatScreen = () => {
  ...
  const [fuelFriendName, setFuelFriendName] = useState('Fuel Friend');
  const [message, setMessage] = useState('');
  const [messages, setMessages] = useState([
    { id: 1, text: "Hey there! I'm your Fuel Friend today. I'll be helping you pump your gas and deliver your groceries to your vehicle.", sender: 'fuelFriend', time: '10:30 AM' },
    { id: 2, text: "Great! I'm at pump #5. When will you arrive?", sender: 'user', time: '10:32 AM' },
    { id: 3, text: "I'm actually already at the station. I'll be at pump #5 in just a minute! Please wait in your vehicle.", sender: 'fuelFriend', time: '10:33 AM' },
  ]);
  const [isTyping, setIsTyping] = useState(false);

  // Get Fuel Friend name from URL parameters
  useEffect(() => {
    const params = new URLSearchParams(location.search);
    const name = params.get('fuelFriendName');
    if (name) {
      setFuelFriendName(name);
    }
  }, [location]);
  ...
  const handleSendMessage = () => {
    if (message.trim()) {
      ...
      // Simulate a reply from the Fuel Friend
      setTimeout(() => {
        setIsTyping(false);
        const fuelFriendReply = {
          id: messages.length + 2,
          text: "I see you now! I'll start pumping your gas right away and bring your groceries to your vehicle when finished.",
          sender: 'fuelFriend',
          time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
        };
        setMessages(prevMessages => [...prevMessages, fuelFriendReply]);
      }, 2000);
    }
  };
      ...
      <div className="px-4 py-3 flex items-center justify-between border-b border-gray-800">
        <div className="flex items-center">
          ...
          <div className="flex items-center">
            ...
            <div>
              <h2 className="font-semibold">{fuelFriendName}</h2>
              <p className="text-xs text-gray-400">Fuel Friend • Online</p>
            </div>
          </div>
        </div>
        <button 
